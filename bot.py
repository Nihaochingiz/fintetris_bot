...
def start(message):
    mess = f'Здравствуйте! Хотите получить бесплатный чек лист "7 вопросов себе для создания капитала"или узнать подробнее о курсе"Финансовый тетрис" Тогда пишите в чат Да?'
...
def get_checklist(message):
    ...
    message_one = f'Двигаемся дальше'
...
def say_bye(message):
            ...
            wks = spreadsheet.worksheet('Телеграм-ФТ')
            today = date.today().strftime("%d.%m.%Y")
            wks.append_row([user_data[4], user_data[3], user_data[2],user_data[1],user_data[0],today])
